...
```{r}
vehicle_ages = c("< 1 Year", "1-2 Year", "> 2 Years")

vehicle_prop = dsDummy %>% 
  filter(Region_Code == "28") %>%
  group_by(Vehicle_Age) %>% 
  summarize(n = n(), 
            interest_count = sum(Response),
            interest_prop = interest_count/n)
vehicle_prop_long = vehicle_prop %>% 
  pivot_longer(cols = c(interest_prop, n), names_to = "Stat", values_to = "num")

ggplot(vehicle_prop_long, aes(x = factor(Vehicle_Age, vehicle_ages), num, fill = Vehicle_Age)) + 
  geom_col() + 
  facet_wrap(.~Stat, scales = "free_y", labeller = as_labeller(c(interest_prop = "Interest Rate",
                                                                 n = "Vehicle Count by Age"))) + 
  scale_fill_manual(values = c("grey70", "grey30", "grey70"), guide = "none") + 
  labs(x = "Vehicle Age", y = "", title = "Owners with Vehicles Over 2 Years Old Most Interested in Insurance", subtitle = "Vehicle age as it relates to interest rate and number of vehicles in region 28", caption = "Source: Health Insurance Cross Sell Prediction - Kaggle") +
  theme_minimal() + 
...
ggplot(anprem, aes(Annual_Premium, int_prop, color = emph)) + 
  ...
  labs(x = "Annual Premium ($1000)", y = "Interest Proportion", 
       title = "Vehicle Insurance Interest Increases Until Annual Premium Hits $50k",
       subtitle = "Insurance interest rates by annual premium in thousands of USD in region 28",
       caption = "Source: Health Insurance Cross Sell Prediction - Kaggle") + 
  theme_classic() + 
